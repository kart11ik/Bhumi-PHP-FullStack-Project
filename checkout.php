...
<body class="goto-here">
	...
	$query = "SELECT p.price, c.quantity FROM cartdb c 
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Insert address
    $query = "INSERT INTO addresses (user_id, fullname, district, street_address, area, pincode, phone, email) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("isssssss", $user_id, $fullname, $district, $street_address, $area, $pincode, $phone, $email);
    $stmt->execute();
    $address_id = $stmt->insert_id; // Get the inserted address ID

    // Get cart items
    $cart_query = "SELECT * FROM cartdb WHERE user_id = ?";
    ...
    if ($total_price > 0) {
        // Insert order
        $order_query = "INSERT INTO orders (user_id, address_id, total_price) VALUES (?, ?, ?)";
        $order_stmt = $conn->prepare($order_query);
        $order_stmt->bind_param("iid", $user_id, $address_id, $total_price);
        $order_stmt->execute();
        $order_id = $order_stmt->insert_id;

        // Insert order items
        $cart_result->data_seek(0); // Reset cart result pointer
        while ($cart_row = $cart_result->fetch_assoc()) {
            ...
            $order_item_stmt->bind_param("iiidd", $order_id, $cart_row['product_id'], $cart_row['quantity'], $cart_row['price'], $cart_row['total_price']);
            $order_item_stmt->execute();
        }

        // Clear cart after placing order
        $delete_cart_query = "DELETE FROM cartdb WHERE user_id = ?";
